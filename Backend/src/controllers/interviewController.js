...
const { updateFormApplicationStatus } = require("../services/formService");
...
async function markMyHallInterviewAbsent(req, res, next) {
  try {
    if (!req.user) {
      const err = new Error("Unauthorized");
      err.status = 401;
    ...
    if (!isAdmin(req.user)) {
      const err = new Error("Only admins can mark interview absent");
      err.status = 403;
      throw err;
    }
    ...
    const rejectionReason = reason || "Absent in interview";

    await markInterviewAbsentByHallId({ interviewId, hallId });

    const meta = await getInterviewApplicationMetaByHallId(interviewId, hallId);
    await updateFormApplicationStatus(
      req.user,
      meta.formId,
      meta.applicationId,
      {
