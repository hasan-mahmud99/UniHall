...
async function updateSeatPlanVisibility(req, res, next) {
  try {
    ...
    if (r !== "examcontroller" && r !== "admin") {
      const err = new Error(
        "Only exam controllers and admins can update visibility"
      );
      err.status = 403;
      throw err;
    }
...
async function deleteSeatPlan(req, res, next) {
  try {
    ...
    if (r !== "examcontroller" && r !== "admin") {
      const err = new Error(
        "Only exam controllers and admins can delete seat plans"
      );
      err.status = 403;
      throw err;
    }
...
async function deleteExamResult(req, res, next) {
  try {
    ...
    if (r !== "examcontroller" && r !== "admin") {
      const err = new Error(
        "Only exam controllers and admins can delete exam results"
      );
      err.status = 403;
      throw err;
    }
