...
const { saveBase64File } = require("../utils/base64Upload");
...
async function manualAssignSeat(req, res, next) {
  try {
    ...
    if (!doc || !(doc.dataUrl || doc.data)) {
      const err = new Error(
        "supporting document is required for manual allocation"
      );
      err.status = 400;
      throw err;
    }

    let manualDocumentUrl = null;
    let manualDocumentName = null;
    const saved = saveBase64File({
      subdir: "pending/manual-allocation-docs",
      fileName: doc.fileName || doc.name || null,
      contentType: doc.contentType || doc.mimeType || null,
      dataUrl: doc.dataUrl || doc.data,
    });
    manualDocumentUrl = saved.url;
    manualDocumentName = saved.name;
