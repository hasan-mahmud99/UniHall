...
async function updateFormHandler(req, res, next) {
  try {
    const data = await updateHallForm(
      req.user,
      req.params.formId,
      req.body || {}
    );
  ...
  try {
    ...
    if (!data) {
      return res
        ...
        .json({
          success: false,
          message: "No active application form available",
        });
    }
    res.json({ success: true, data });
  } catch (err) {
...
async function getMyApplicationDetails(req, res, next) {
  try {
    const data = await getStudentApplicationDetails(
      req.user,
...
async function updateApplicationStatus(req, res, next) {
  try {
    const data = await updateFormApplicationStatus(
      req.user,
      req.params.formId,
      req.params.applicationId,
      req.body || {}
...
async function recalculateApplicationScores(req, res, next) {
  try {
    const data = await recalculateFormApplicationScores(
      req.user,
      req.params.formId
    );
    res.json({ success: true, data });
