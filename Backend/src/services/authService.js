...
function parseUndergradStudentId(studentId) {
  const match = String(studentId || "")
    .toUpperCase()
    .match(/^([A-Z]{3})(\d{2})(\d{2})(\d{3})([MF])$/);

  if (!match) return null;

  return {
    hallCode: match[1],
    sessionFragment: match[2],
    departmentCode: match[3],
    roll: match[4],
    genderSuffix: match[5],
  };
}

function parseGraduateUniversityId(universityId) {
  const match = String(universityId || "")
    .toUpperCase()
    .match(/^([A-Z]{3})([A-Z0-9]{1,16})([MF])$/);
  ...
  return {
    hallCode: match[1],
    body: match[2],
    genderSuffix: match[3],
  };
}

async function validateStudentIdAgainstHall(studentId) {
  const upper = String(studentId || "")
    .trim()
    .toUpperCase();
  if (!upper) {
  ...
  const parsedUndergrad = parseUndergradStudentId(upper);
  if (parsedUndergrad) {
    const hall = await findHallByCodeWithGender(parsedUndergrad.hallCode);
    if (!hall) {
      throw new Error("Student ID prefix does not match any hall");
    }
    const expectedGender = hall.gender === "MALE" ? "M" : "F";
    if (parsedUndergrad.genderSuffix !== expectedGender) {
      throw new Error(
        `Student ID must end with ${expectedGender} for ${hall.gender.toLowerCase()} halls`
      );
    }
    return { hall, level: "undergraduate" };
  }

  const parsedGraduate = parseGraduateUniversityId(upper);
  if (parsedGraduate) {
    const hall = await findHallByCodeWithGender(parsedGraduate.hallCode);
    if (!hall) {
      throw new Error("University ID prefix does not match any hall");
    }
    const expectedGender = hall.gender === "MALE" ? "M" : "F";
    if (parsedGraduate.genderSuffix !== expectedGender) {
      throw new Error(
        `Student ID must end with ${expectedGender} for ${hall.gender.toLowerCase()} halls`
      );
    }
    return { hall, level: "graduate" };
  }

  throw new Error(
    "Student ID must be HALLCODE + session(2 digits) + department(2 digits) + roll(3 digits) + gender (M/F)"
  );
}
...
async function registerStudent({
  ...
  const parsedId = parseUndergradStudentId(normalizedStudentId);
  if (!parsedId) {
    throw new Error(
      "Student ID must be HALLCODE + session(2 digits) + department(2 digits) + roll(3 digits) + gender (M/F)."
    );
  ...
  const hall = await findHallByCodeWithGender(parsedId.hallCode);
  if (!hall) {
    throw new Error("Student ID prefix does not match any hall");
  }

  const expectedGender = hall.gender === "MALE" ? "M" : "F";
  if (parsedId.genderSuffix !== expectedGender) {
    throw new Error(
      `Student ID must end with ${expectedGender} for ${hall.gender.toLowerCase()} halls`
    );
  }
...
async function registerGraduateStudent({
  ...
  const parsedGraduateId = parseGraduateUniversityId(normalizedUniversityId);
  if (!parsedGraduateId) {
    throw new Error(
      "Student ID must start with a hall code (3 letters) and end with gender (M/F)"
    );
  }
  ...
  const hall = await findHallByCodeWithGender(parsedGraduateId.hallCode);
  if (!hall) {
    throw new Error("University ID prefix does not match any hall");
  }

  const expectedGender = hall.gender === "MALE" ? "M" : "F";
  if (parsedGraduateId.genderSuffix !== expectedGender) {
    throw new Error(
      `Student ID must end with ${expectedGender} for ${hall.gender.toLowerCase()} halls`
    );
  }
...
async function updateStudentProfile(user, profileInput = {}) {
  ...
  if (!isGraduateAccount && looksLikeUndergradRoll) {
    const parsed = parseUndergradStudentId(universityId);
    if (!parsed) {
      const err = new Error(
        "Student ID must be HALLCODE + session(2 digits) + department(2 digits) + roll(3 digits) + gender (M/F)."
      );
    ...
    const hall = await findHallByCodeWithGender(parsed.hallCode);
    if (!hall) {
      const err = new Error("Student ID prefix does not match any hall");
      err.status = 400;
      throw err;
    }

    const expectedGender = hall.gender === "MALE" ? "M" : "F";
    if (parsed.genderSuffix !== expectedGender) {
      const err = new Error(
        `Student ID must end with ${expectedGender} for ${hall.gender.toLowerCase()} halls`
      );
      err.status = 400;
  ...
  // Graduate accounts: infer hallId from the first 3 letters of universityId and enforce gender suffix.
  // Do not infer department/session.
  if (isGraduateAccount && universityId) {
    const parsed = parseGraduateUniversityId(universityId);
    if (!parsed) {
      const err = new Error(
        "Student ID must start with a hall code (3 letters) and end with gender (M/F)"
      );
    ...
    const hall = await findHallByCodeWithGender(parsed.hallCode);
    if (!hall) {
      const err = new Error("University ID prefix does not match any hall");
      err.status = 400;
      throw err;
    }
...
async function updateProfile(userId, payload = {}) {
  ...
  switch (dbUser.role) {
    case "STUDENT":
      ...
      break;
    case "ADMIN":
      await updateAdminProfile(dbUser, profileInput);
      break;
    case "STAFF":
      await updateStaffProfile(dbUser, profileInput);
      break;
    case "EXAM_CONTROLLER":
      await updateExamControllerProfile(dbUser, profileInput);
      break;
    default: {
      const err = new Error("Profile updates are not supported for this role");
      err.status = 400;
      throw err;
    }
  }
