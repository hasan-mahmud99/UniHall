...
async function listSeatPlans(filters = {}) {
  ...
  if (!department && Array.isArray(departments) && departments.length) {
    where += ` AND sp.department IN (${departments.map(() => "?").join(",")})`;
...
async function listExamResults(filters = {}) {
  ...
  if (!department && Array.isArray(departments) && departments.length) {
    where += ` AND er.department IN (${departments.map(() => "?").join(",")})`;
    params.push(...departments);
  }
...
async function updateExamResult(resultId, data) {
  const pool = await initPool();
  const { semester, academicYear, department, title, description, isVisible } =
    data;
...
async function getDistinctAcademicYears() {
  ...
  return ["2021-2022", "2022-2023", "2023-2024", "2024-2025", "2025-2026"];
}
