...
async function markInterviewAbsentByHallId({ interviewId, hallId }) {
  if (!interviewId || !hallId) {
    const err = new Error("interviewId and hallId are required");
    err.status = 400;
    throw err;
  ...
  if (!result.affectedRows) {
    const err = new Error("Interview not found or already processed");
    err.status = 404;
    throw err;
  }

  return { interviewId, status: 2 };
}
