...
async function createStudentUser({
  ...
  try {
    ...
    await connection.query(
      ...
      [userId, email, hashedPassword, name, isEmailVerified ? 1 : 0]
    );
...
async function findHallByCodeWithGender(code) {
  const pool = await getPool();
  const [rows] = await pool.query(
    "SELECT hallId, hallCode, gender FROM halls WHERE hallCode = ? LIMIT 1",
    [code]
  );
  return rows[0] || null;
}
...
async function findHallByCode(code) {
  const hall = await findHallByCodeWithGender(code);
  return hall ? hall.hallId : null;
}
...
async function findStaffProfileByUserId(userId) {
  const pool = await getPool();
  const [rows] = await pool.query(
    `SELECT sp.userId, sp.hallId, h.hallName, h.hallCode, sp.phone, sp.photoUrl, sp.created_at, sp.updated_at
...
async function upsertStaffProfile(userId, profile) {
  const pool = await getPool();
  const { hallId = null, phone = null, photoUrl = null } = profile;

  await pool.query(
    `INSERT INTO staff_profiles (userId, hallId, phone, photoUrl, created_at)
     VALUES (?, ?, ?, ?, NOW())
     ON DUPLICATE KEY UPDATE
       hallId = VALUES(hallId),
       phone = VALUES(phone),
...
async function findExamControllerProfileByUserId(userId) {
  ...
  const [rows] = await pool.query(
    `SELECT ep.userId, ep.contactExt AS contact, ep.phone, ep.photoUrl, ep.department, ep.created_at, ep.updated_at
...
async function upsertExamControllerProfile(userId, profile) {
  ...
  await pool.query(
    `INSERT INTO exam_controller_profiles (userId, contactExt, phone, photoUrl, department, created_at)
     VALUES (?, ?, ?, ?, ?, NOW())
     ON DUPLICATE KEY UPDATE
       ...
       department = VALUES(department),
       updated_at = NOW()`,
    [userId, contact, phone, photoUrl, department]
  );
