...
export default function SeatPlans() {
  ...
  const [name, setName] = useState('Seat Plan')
  const [content, setContent] = useState('')
  const list = api.listSeatPlanUploads({ hallId })
  const submit = (e) => {
    e.preventDefault()
    api.createSeatPlanUpload({ hallId, name, content })
    setContent('')
  }
  return (
    <div className="grid gap-4">
      <form onSubmit={submit} className="bg-white border rounded p-4 grid gap-2">
        <h2 className="text-lg font-semibold">Upload Seat Plan</h2>
