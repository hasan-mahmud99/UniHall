...
export default function Results() {
  ...
  const loadData = async () => {
    try {
      ...
      setFilterOptions(
        filtersRes.data || { departments: [], academicYears: [], semesters: [] }
      );
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(
        "Failed to upload: " + (error.response?.data?.message || error.message)
      );
    } finally {
      setUploading(false);
    }
  ...
  const handleToggleVisibility = async (resultId, currentVisibility) => {
    if (
      !confirm(
        `Are you sure you want to ${
          currentVisibility ? "hide" : "publish"
        } this result?`
      )
    ) {
      return;
    ...
    try {
      ...
      alert(
        `Result ${!currentVisibility ? "published" : "hidden"} successfully!`
      );
      loadData();
      try {
        window.dispatchEvent(new Event("examStatsChanged"));
  ...
  return (
    <div className="grid gap-6">
      ...
      <form
        ...
        className="rounded-2xl border border-white/60 bg-gradient-to-r from-white/95 via-sky-50/80 to-emerald-50/70 p-6 shadow-sm backdrop-blur"
      >
          ...
          <div className="md:col-span-2">
            <label className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-gray-700">
                Make visible to admins immediately (Results are never visible to
                students)
              </span>
            </label>
          </div>
            ...
            {results.map((result) => (
              ...
              >
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500">
                      {result.department} • {result.semester}{" "}
                      {result.academicYear}
                    </p>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() =>
                      handleToggleVisibility(result.resultId, result.isVisible)
                    }
