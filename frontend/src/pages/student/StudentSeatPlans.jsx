...
import { getSeatPlans } from "../../lib/examApi";
...
export default function StudentSeatPlans() {
  ...
  const loadSeatPlans = async () => {
    setLoading(true);
    try {
      // Backend enforces: students only see visible seat plans for their own department.
      const res = await getSeatPlans();
      setSeatPlans(res.data || []);
    } catch (error) {
      console.error("Failed to load seat plans:", error);
      alert(
        "Failed to load seat plans: " +
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-black font-bold mt-2">
          View your exam seating arrangements
        </p>
      </div>
            ...
            {seatPlans.map((plan) => (
              ...
              >
                ...
                <a
                  ...
                  download={
                    plan.fileName || `${plan.examName || "seat-plan"}.pdf`
                  }
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-800">
              Please check your seat plan carefully before the exam. Make note
              of your room number, seat number, and exam time. Arrive at least
              15 minutes before the scheduled exam time.
            </p>
          </div>
        </div>
      </div>
