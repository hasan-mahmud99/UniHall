...
export default function StudentRenewals() {
  ...
  const MAX_RENEWAL_UPLOAD_BYTES = 10 * 1024 * 1024;
  ...
  const hasProof = Boolean(attachmentFile);
  ...
  async function handleSubmit(e) {
    ...
    setError("");
    try {
      if (attachmentFile && attachmentFile.size > MAX_RENEWAL_UPLOAD_BYTES) {
        throw new Error("File too large. Maximum size is 10MB.");
      }

      const dataUrl = attachmentFile ? await fileToDataUrl(attachmentFile) : "";
      const uploadRes = await apiClient.post("/uploads/renewals/base64", {
        fileName: attachmentFile?.name || "proof",
        contentType: attachmentFile?.type || "application/octet-stream",
        dataUrl,
      });
      const uploaded = uploadRes?.data?.file || null;
      if (!uploaded?.url) {
      ...
      const payload = {
        ...
        attachment: {
          name: uploaded.name || attachmentFile?.name || "proof",
          type: uploaded.type || attachmentFile?.type || "application/octet-stream",
          url: uploaded.url,
        },
      };
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                disabled={formDisabled}
              />
              {attachmentFile?.size > MAX_RENEWAL_UPLOAD_BYTES ? (
