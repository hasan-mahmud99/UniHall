...
export default function ManageAllocations() {
  ...
  const submitManualAllocation = async () => {
    ...
    if (!manualDocument) {
      alert("Supporting document is required for manual allocation");
      return;
    }

    let document = null;
    if (manualDocument.size > 5 * 1024 * 1024) {
      alert("Document is too large. Maximum size is 5MB.");
      return;
    }
    const dataUrl = await readFileAsDataUrl(manualDocument);
    document = {
      fileName: manualDocument.name,
      contentType: manualDocument.type,
      dataUrl,
    };
  ...
  return (
    ...
    >
      ...
      <section className="mt-4 rounded-2xl border border-white/60 bg-gradient-to-r from-white/95 via-sky-50/80 to-emerald-50/70 p-4 shadow-sm backdrop-blur space-y-3">
