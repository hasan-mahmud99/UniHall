...
export default function Interviews() {
  ...
  const markAbsent = async (interviewId) => {
    if (!interviewId) return;

    const ok = window.confirm(
      "Mark this student as ABSENT for the interview?\n\nThis will reject the application and remove this interview from the list."
    );
    if (!ok) return;

    const promptValue = window.prompt(
      "Rejection reason to show the student (optional):",
      "Absent in interview"
    );
    if (promptValue === null) return;
    const rejectionReason =
      String(promptValue || "").trim() || "Absent in interview";

    setSaving((s) => ({ ...s, [interviewId]: true }));
    setError("");
    try {
      await api.put(`/interviews/${encodeURIComponent(interviewId)}/absent`, {
        rejectionReason,
      });
      setItems((prev) =>
        (prev || []).filter((it) => it.interviewId !== interviewId)
      ...
      setError(e?.response?.data?.message || "Failed to mark absent");
    } finally {
      setSaving((s) => ({ ...s, [interviewId]: false }));
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 rounded-2xl border border-white/60 bg-gradient-to-r from-white/95 via-sky-50/80 to-emerald-50/70 p-4 shadow-sm backdrop-blur">
              ...
              <div className="mt-3">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={!!saving[r.id]}
                    className="w-full sm:w-auto px-3 py-2 text-sm rounded bg-red-600 text-white hover:bg-red-700 disabled:opacity-60"
                  >
                    Mark Absent
                  </button>
                </div>
      ...
      {/* Desktop table */}
      <div className="mt-4 border border-gray-200 rounded hidden md:block bg-white overflow-hidden">
                ...
                filteredRows.map((r, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 max-w-[220px] whitespace-normal break-words">
                        {r.venue || "—"}
                      </td>
                      <td className="px-3 py-2 max-w-[180px]">
                        <button
                          type="button"
                          onClick={() => openDetails(r.id)}
                          className="block max-w-full text-left underline truncate"
                          title="View submitted application form"
                      ...
                      <td className="px-3 py-2 whitespace-nowrap">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <button
                            ...
                            onClick={() => saveAndConfirmScore(r.id)}
                            disabled={!!saving[r.id]}
                            className="px-3 py-1.5 text-xs rounded bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-60 whitespace-nowrap"
                          >
                            {saving[r.id] ? "Saving…" : "Save & Confirm"}
                          </button>
                          <button
                            ...
                            onClick={() => markAbsent(r.id)}
                            disabled={!!saving[r.id]}
                            className="px-3 py-1.5 text-xs rounded bg-red-600 text-white hover:bg-red-700 disabled:opacity-60 whitespace-nowrap"
                          >
                            Mark Absent
                          </button>
                        </div>
