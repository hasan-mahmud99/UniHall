...
function parseDateTimeMaybe(value) {
  if (!value) return null;
  ...
  // MySQL DATETIME sometimes comes as "YYYY-MM-DD HH:mm:ss"
...
function formatDateTime(value) {
  const parsed = parseDateTimeMaybe(value);
  if (!parsed) return "";
...
export default function Profile() {
  ...
  const canEditProfile = useMemo(
    () => ["student", "admin", "staff", "examcontroller"].includes(user?.role),
    [user]
  );
  ...
  useEffect(() => {
    ...
    async function fetchActiveForm() {
      ...
      try {
        ...
        if (!ignore) setActiveForm(data?.data || null);
      } catch (err) {
        if (!ignore) {
          setActiveForm(null);
          const message =
            err.response?.data?.message ||
            "No active form available right now.";
          setActiveFormError(message);
        }
      } finally {
        if (!ignore) setActiveFormLoading(false);
      }
    }
  ...
  }, [user?.role, user?.hallId, user?.studentId]);
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (user?.role === "student") {
        ...
        profilePayload.contact = formState.contact.trim() || null;
        profilePayload.phone = formState.phone.trim() || null;
      }
    ...
    <div className="space-y-6">
      ...
      {user?.role === "student" && (
        <Section title="Application Window">
          {activeFormLoading ? (
            <div className="text-sm text-gray-600">Loading form window…</div>
          ) : activeForm ? (
            <div className="space-y-3">
              <InfoRow
                label="Form"
                value={activeForm.title || activeForm.name}
              />
              <InfoRow
                label="Publish from"
                value={
                  activeForm?.applicationStartDate
                    ? formatDateTime(activeForm.applicationStartDate)
                    : "Immediately"
                }
              />
              <InfoRow
                label="Publish until"
                value={
                  activeForm?.applicationDeadline
                    ? formatDateTime(activeForm.applicationDeadline)
                    : "No deadline"
                }
              />
            </div>
            ...
            <div className="text-sm text-gray-600">
              {activeFormError || "No active form available right now."}
            </div>
          )}
        </Section>
      )}
        ...
        <Section title="Profile Details">
          {canEditProfile && isEditing ? (
            ...
            <div className="space-y-4">
              ...
              {user?.role === "staff" && (
                <>
                  <InfoRow label="Hall" value={profile?.hallName} />
                </>
              )}
              {isStudent && (
                <>
